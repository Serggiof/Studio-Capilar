...
const Plasma = {
  render: (el) => {
    const sesiones = DB.sesionesPlasma();

    el.innerHTML = `
      <div class="modulo-header">
        <h1>Sesiones de Plasma</h1>
        <button class="btn-primary" onclick="Plasma.abrirNuevoPlan()">+ Nuevo plan</button>
      </div>

      <div class="plasma-grid">
        ${sesiones.length === 0
          ? '<p class="empty-state">Sin planes de plasma registrados.</p>'
          : sesiones.map(s => Plasma._card(s)).join("")}
      </div>
    `;
  },
      ...
      <div class="plasma-card ${enAlerta ? "plasma-alerta" : ""}">
          ...
          ${s.sesiones.length === 0
            ...
            : s.sesiones.map(ses => `
                <div class="sesion-row">
                  ...
                  <span class="sesion-nota">${ses.notas || "—"}</span>
                </div>`).join("")}
        </div>
        ...
        <div class="plasma-footer">
          ...
          <div style="display:flex;gap:6px">
            <button class="btn-sm" onclick="Plasma.registrarSesion('${s.id}')">+ Sesión</button>
            <button class="btn-sm" onclick="Plasma.eliminarPlan('${s.id}')">✕</button>
          </div>
        </div>
      </div>`;
  },
